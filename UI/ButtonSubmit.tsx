...
export function ButtonSubmit({ onPress, title, className, textClassName, isPending, titleWhenIsPending }: Props) {
  return (
    <TouchableOpacity
      onPress={onPress}
      className={`fixed left-0 right-0 bottom-0 bg-blue-500 flex flex-row w-full p-4 rounded-lg items-center justify-center `}
    >
      <View className="m-auto flex flex-row items-center gap-2">
        <Text className={`text-lg text-zinc-100 font-medium ${textClassName}`} >
          {titleWhenIsPending ? titleWhenIsPending : title}
        </Text>
        {isPending ? <Icon color="#fff" icon={Loader} className="animation-spin" /> : <Icon color="#fff" icon={ArrowRight} />}
      </View>
    </TouchableOpacity>
  );
}